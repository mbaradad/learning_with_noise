...
import vtab_eval.task_adaptation.trainer as trainer
...
def model_fn(features, mode, params):
  ...
  train_linear_only = params["train_linear_only"]
  ...
  if is_legacy_hub_module:
    ...
    all_layers = None
    try:
        all_layers = module(features["image"],
                            signature=params["hub_module_signature"],
                            as_dict=True)
        pre_logits = all_layers[finetune_layer]
    except Exception as e:
        print("Desired signature: {} Available signatures: {}".format(module.get_signature_names(), params["hub_module_signature"]))
        if not all_layers is None:
            print("Finetune layer {} not available!.format(finetune_layer. Available are: ")
            print(all_layers.keys())
        raise e
  else:
    try:
        module = hub.load(hub_module).signatures[params["hub_module_signature"]]
        pre_logits = module(features["image"])['output_0']
    except Exception as e:
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    train_op, gradients = trainer.get_train_op(loss,
    ...
    for g in gradients:
        for gradient in g:
            predictions['gradient:' + gradient.name] = gradient
    predictions['embeddings'] = pre_logits
    return spec_type(mode=mode, loss=loss, train_op=train_op, predictions=predictions)
  else:
